...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x1 = x_train.flatten(start_dim=1)
    x2 = x_test.flatten(start_dim=1)
    for i in range(num_train):
        for j in range(num_test):
            dists[i, j] = (x1[i] - x2[j]).pow(2).sum().sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x2 = x_test.flatten(start_dim=1)
    for i in range(num_train):
        dists[i, :] = (x2 - x1[i]).pow(2).sum(dim=1).sqrt()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ip = torch.mm(x1, x2.t())
    sm = x1.pow(2).sum(dim=1, keepdim=True) + x2.pow(2).sum(dim=1)
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    vals, idx = torch.topk(dists.t(), k, dim=1, largest=False)
    y_pred, _ = torch.mode(y_train[idx], dim=1)
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            xs_train = torch.cat(x_train_folds[:i] + x_train_folds[i + 1:])
            ys_train = torch.cat(y_train_folds[:i] + y_train_folds[i + 1:])
            xs_validate = x_train_folds[i]
            ys_validate = y_train_folds[i]
            classifier = KnnClassifier(xs_train, ys_train)
            accuracy = classifier.check_accuracy(xs_validate, ys_validate, k)
            k_to_accuracies[k].append(accuracy)
        # k_to_accuracies[k] = k_to_accuracies[k] / num_folds
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
