...
def nn_forward_backward(
    ...
    vals, _ = scores.max(dim=1, keepdim=True)
    probs = scores - vals
    probs = torch.exp(probs)
    probs = probs / probs.sum(dim=1, keepdim=True)
    prob = probs[torch.arange(N), y]
    prob = -1 * torch.log(prob)
    loss = prob.sum() / N
    ...
    dS2 = probs
    dS2[torch.arange(N), y] -= 1
    dW2 = torch.mm(h1.t(), dS2) / N
    dW2 += reg * 2 * W2
    
    dB2 = dS2.sum(dim=0) / N

    dH1 = torch.mm(dS2, W2.t())

    dS1 = h1.clamp(0, 1)
    dS1 = dS1.ceil()
    dS1 = dH1 * dS1

    dW1 = torch.mm(X.t(), dS1) / N
    dW1 += reg * 2 * W1
