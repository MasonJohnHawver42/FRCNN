...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    x = torch.tensor([ i - (i % 10) + 10 for i in range(start, stop, 10) ], dtype=torch.float64) if (stop - start) >= 10 else torch.tensor([], dtype=torch.float64)
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    y = x.clone()
    print(x.shape)
    _, idx_cols = x.min(dim=1)
    idx_rows = torch.arange(x.shape[0])
    y[idx_rows, idx_cols] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
